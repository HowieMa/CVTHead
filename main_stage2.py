...
def main(args):
    ...
    set_logger(os.path.join(out_dir, 'train_stage2.log'))
    ...
    logging.info("[main] Load pre-trained from stage 1 ...")

    ckpt = torch.load("{}/model_G.pt".format(out_dir), map_location="cpu")
    generator.load_state_dict(ckpt["model"])
    ckpt = torch.load("{}/model_D.pt".format(out_dir), map_location="cpu")
    ...
    # Intialize training
    trainer = Trainer(generator, discriminator, optimizer_G, optimizer_D, cfg, device, out_dir, stage=2)
    ...
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Resume >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    try:
        load_dict_G = checkpoint_G.load("model_G_s2.pt")
    except FileNotFoundError:
        load_dict_G = dict()
        print("No checkpoint G !!!")
    try:
        load_dict_D = checkpoint_D.load("model_D_s2.pt")
    except FileNotFoundError:
        load_dict_D = dict()
        print("No checkpoint D !!!")
    ...
    for epoch in range(start_epoch + 1, epochs // 10 + 1):
        ...
        if rank == 0:
            ...
            checkpoint_G.save('model_G_s2.pt', **checkpoint_scalars)
            checkpoint_D.save("model_D_s2.pt", **checkpoint_scalars)
        ...
        if epoch % cfg["training"]["eval_per_epoch"] == 0:
            ...
            if model_selection_sign * (metric_val - metric_val_best) > 0:
                ...
                if rank == 0:
                    ...
                    checkpoint_G.save('model_best_G_s2.pt', **checkpoint_scalars)
