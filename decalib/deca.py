...
class DECA(nn.Module):
    def __init__(self, config=None, device='cuda'):
        ...
        # !! Our framework gets rid of differentiable rendering
    ...
    def _create_model(self, model_cfg):
        ...
        # !! for simplicity, we remove the detail encoder of DECA
    ...
    def encode(self, images, use_detail=True):
        ...
        # shape (100) / texture (50) / expression (50) / pose (6) / camera (3) / light (27)
    ...
    # @torch.no_grad()
    def decode(self, codedict, rendering=False, iddict=None, vis_lmk=True, return_vis=True, use_detail=True,
        ...
        # Coarse { shape (beta) + expression (Phi) + pose (theta) }
        # verts,        (B, 5023, 3)  after global rotation in face scale
        # landmarks3d   (B, 68, 3)      in the same coord as verts 
        # landmarks2d   (B, 68, 3) 
        ...
        # cam: {s,tx,ty}
        # [X, Y, Z] -> s * (X+tx, (-Y+ty), -Z)
        landmarks2d = util.batch_orth_proj(landmarks2d, codedict['cam'])[:,:,:2]    # (B, 68, 2)
        ...
        trans_verts[:,:,1:] = -trans_verts[:,:,1:]      # !!! Rotate Y  [X, Y, Z] -> s * (X+tx, -(Y+ty), -Z)
        ...
        #     albedo = self.flametex(codedict['tex'])         # (B, 3, 256, 256) U-V map
        # else:
        #     albedo = torch.zeros([batch_size, 3, self.uv_size, self.uv_size], device=images.device)     # (B, 3, 256, 256) U-V map
        ...
        #     trans_verts = transform_points(trans_verts, tform, points_scale, [h, w])        # verts in original image scale
        ...
        #     # ['images', 'albedo_images', 'alpha_images', 'pos_mask', 'shading_images', 'grid', 'normals', 'normal_images', 'transformed_normals']
        ...
        #     # jaw pose + exp weights + detail_code --> (B, 1, 256, 256) displacement map

        #     if iddict is not None:
        #         uv_z = self.D_detail(torch.cat([iddict['pose'][:,3:], iddict['exp'], codedict['detail']], dim=1))
            
        #     uv_detail_normals = self.displacement2normal(uv_z, verts, ops['normals'])       # (B, 3, 256, 256)
        #     uv_shading = self.render.add_SHlight(uv_detail_normals, codedict['light'])      # (B, 3, 256, 256)
        ...
        #     detail_normal_images = F.grid_sample(uv_detail_normals, grid, align_corners=False)*alpha_images # (B, 3, H, W)
        ...
        #     opdict['uv_texture_gt'] = uv_texture_gt     # (B, 3, 256, 256)
        #     visdict = {
        #         'inputs': images,       # (B, 3, 224, 224)
        #         'landmarks2d': util.tensor_vis_landmarks(images, landmarks2d),      # (B, 3, 224, 224)
