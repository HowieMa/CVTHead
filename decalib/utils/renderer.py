...
class Pytorch3dRasterizer(nn.Module):
    ...
    def forward(self, vertices, faces, attributes=None, h=None, w=None):
        ...
        attributes = attributes.clone();            # F 个faces, 每个face 有D=12个属性
        attributes = attributes.view(attributes.shape[0]*attributes.shape[1], 3, attributes.shape[-1])      # (B * F, 3, D)
        ...
        idx = pix_to_face.view(N * H * W * K, 1, 1).expand(N * H * W * K, 3, D) # (B, H, W, K) -> (B H W K, 1, 1)  -> (B H W K, 3, D)
        pixel_face_vals = attributes.gather(0, idx).view(N, H, W, K, 3, D)      # (B H W K, 3, D)

        # (B, H, W, K, 3, 1) * (B, H, W, K, 3, D) --> (B, H, W, K=1, D)
        pixel_vals = (bary_coords[..., None] * pixel_face_vals).sum(dim=-2) # 总共HW个pixel, 每个pixel对应K个face，每个face对应D个属性
        pixel_vals[mask] = 0  # Replace masked values in output.
        pixel_vals = pixel_vals[:,:,:,0].permute(0,3,1,2)       # (B, H, W, D) --> (B, D, H, W)
        pixel_vals = torch.cat([pixel_vals, vismask[:,:,:,0][:,None,:,:]], dim=1)       # (B, D+1, H, W)
        return pixel_vals
...
class SRenderY(nn.Module):
    def __init__(self, image_size, obj_filename, uv_size=256, rasterizer_type='pytorch3d'):
        ...
        if rasterizer_type == 'pytorch3d':
            ...
            uvcoords = aux.verts_uvs[None, ...]      # (1, V=5118, 2), 每个vertex 对应 2D UV map 上的坐标
            uvfaces = faces.textures_idx[None, ...]         # (N, F=9976, 3)   每个face 对应的3个V (max: 5118)
        ...
        uvcoords = torch.cat([uvcoords, uvcoords[:,:,0:1]*0.+1.], -1) #[bz, V=5023, 3]  (u, v, 1) 坐标齐次化, range: 0-1
        uvcoords = uvcoords*2 - 1;      # range [-1, 1]
        uvcoords[...,1] = -uvcoords[...,1]      # 更改Y坐标
        face_uvcoords = util.face_vertices(uvcoords, uvfaces)           # (B, F=9976, 3, 3) 每个face对应3个verts, 每个verts 对应一个 (u,v,1)
        self.register_buffer('uvcoords', uvcoords)
        self.register_buffer('uvfaces', uvfaces)
        self.register_buffer('face_uvcoords', face_uvcoords)

        # >>>>>>>>>>>>>>>>>>>>>>shape colors, for rendering shape overlay >>>>>>>>>>>>>>>>>>>>>>  
        colors = torch.tensor([180, 180, 180])[None, None, :].repeat(1, faces.max()+1, 1).float()/255.      # (1, V=5023, 3)
        face_colors = util.face_vertices(colors, faces)         # 每个face对应3个verts, 每个verts 对应一个(R,G,B)
    ...
    def forward(self, vertices, transformed_vertices, albedos, lights=None, h=None, w=None, light_type='point', background=None):
        ...
        # >>>>>>>>>>>>>>>>>>>>> attributes >>>>>>>>>>>>>>>>>>>>>
        face_vertices = util.face_vertices(vertices, self.faces.expand(batch_size, -1, -1))     # (B, 9976, 3, 3)
        # 每个face 对应3个verts，每个verts 的(x,y,z) 坐标
        normals = util.vertex_normals(vertices, self.faces.expand(batch_size, -1, -1));         # (B, 5023, 3)
        # 每个verts 的normal
        ...
        attributes = torch.cat([self.face_uvcoords.expand(batch_size, -1, -1, -1), 
                                ...
                                face_normals], 
                                -1)     # (B, F=9976, 3, D=12)   [u,v,1] + [x,y,z] normal of transformed verts + [x,y,z] verts coordinates + [x,y,z] normal
        ...
        ####
        # >>>>>>>>>>>>>>> vis mask >>>>>>>>>>>>>>>
        alpha_images = rendering[:, -1, :, :][:, None, :, :].detach()       # (B, 1, H, W)  binary

        # >>>>>>>>>>>>>>> albedo >>>>>>>>>>>>>>>
        uvcoords_images = rendering[:, :3, :, :]            # (B, 3, H, W)   val:[-1, 1] 图片上每个pixel对应的vertex，vertex 对应在 UV map 上的坐标
        grid = (uvcoords_images).permute(0, 2, 3, 1)[:, :, :, :2]       # (B, 3, H, W) -> (B, H, W, 3) --> (B, H, W, 2)
        albedo_images = F.grid_sample(albedos, grid, align_corners=False)       # sample color from UV map
        # albedos (B, 256, 256, 3) + (B, 224, 224, 2) --> (B, 3, 224, 224)

        # visible mask for pixels with positive normal direction
        transformed_normal_map = rendering[:, 3:6, :, :].detach()           # (B, 3, H, W) normal of transformed verts
    ...
    def world2uv(self, vertices):
        ...
        '''
        batch_size = vertices.shape[0]
        face_vertices = util.face_vertices(vertices, self.faces.expand(batch_size, -1, -1))     # (B, F, 3, 3) for each face, 3 verts with 3 xyz
