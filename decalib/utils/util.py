...
def face_vertices(vertices, faces):
    ...
    faces = faces + (torch.arange(bs, dtype=torch.int32).to(device) * nv)[:, None, None]        # (B, num_of_faces, 3)
...
def batch_orth_proj(X, camera):
    ...
    X_trans = X[:, :, :2] + camera[:, :, 1:]        # (B, V, 2)     [X+tx, Y+ty]
    X_trans = torch.cat([X_trans, X[:,:,2:]], 2)    # (B, V, 3)     [X+tx, Y+ty, Z]
    shape = X_trans.shape
    Xn = (camera[:, :, 0:1] * X_trans)              # (B, V, 3)     s * [X+tx, Y+ty, Z]
    return Xn
...
def tensor_vis_landmarks(images, landmarks, gt_landmarks=None, color = 'g', isScale=True):
    ...
    # landmarks, (B, 68, 2) or (B, 68, 3)
    # visualize landmarks
    vis_landmarks = []
    images = images.cpu().numpy()
    predicted_landmarks = landmarks.detach().cpu().numpy()  # (B, 68, 2)
    ...
    for i in range(images.shape[0]):
        ...
        # (3, H, W) -> (H, W, 3) --> 
        image = image.transpose(1,2,0)[:,:,[2,1,0]].copy(); image = (image*255)     # (224, 224, 3)
        if isScale:
            predicted_landmark = predicted_landmarks[i]     # (68, 2), val: [-1, 1]
            # (x, y) * W/2 + W/2
