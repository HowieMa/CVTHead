...
class FaceParsing(nn.Module):
    def __init__(self, ckpt_path="assets/79999_iter.pth"):
        super().__init__()
    ...
    def forward(self, x):
        # (B, 3, H=256, W=256)m val:[-1, 1]
        b, _, h, w = x.shape
        x = 0.5 * (x + 1)       # [-1, 1] --> [0, 1]

        x = (x - self.mean[None, :, None, None]) / self.std[None, :, None, None]
        x = F.interpolate(x, size=(512, 512), mode='bilinear')
        y = self.net(x)[0]      # (B, C=19, 512, 512)
        y = F.interpolate(y, size=(h, w), mode='bilinear')      # (B, C, 256, 256)

        labels = y.argmax(1)

        mask = torch.zeros(b, len(self.mask_types.keys()), h, w, dtype=x.dtype, device=x.device)        # (B, 5, H, W)
